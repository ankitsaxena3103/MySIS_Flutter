...
class ScanUnitShiftViewState extends State<ScanUnitShiftView> {
  ...
  late List<UnitDutyPost> unitDutyPosts;
  late List<UnitShiftDetail> unitShiftDetails;
  late List<UserPosting> userPostingsData;
                                    ...
                                    decoration: BoxDecoration(
                                      ...
                                      boxShadow: [
                                        BoxShadow(
                                          ...
                                          offset:
                                          Offset(-pathS / 12, pathS / 12),
                                        ),
                                      ],
                                    ),
                                    child: Padding(
                                      padding: EdgeInsets.only(
                                          left: pathS / 5, right: pathS / 5),
                                      child: Text(
                                        'mark_escort_duty'.tr(),
                                        style: TextStyle(
                                          color: isDarkMode
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: MobileScanner(
                          ...
                          for (final barcode in barcodeCapture.barcodes) {
                            ...
                            if (raw != null) {
                              ...
                              if (qrCode != null) {
                                ...
                                await getUnitPostData(qrCode);
                              } else {
                                showPopupAlert(
                                    'alert'.tr(), 'invalid_qrcode'.tr());
                        ...
                        fit: BoxFit.cover,
                        // Ensure the scanner fills the container
                                ...
                                children: [
                                  ...
                                  ElevatedButton(
                                    ...
                                    child: Text(
                                      ...
                                      style: TextStyle(
                                        color:
                                        isDarkMode ? redColor1 : redColor3,
                                        fontSize: pathS / 5.5,
                                        fontWeight: FontWeight.w400,
                                        fontFamily: 'Roboto',
                                      ),
                  ...
                  child: AlertPopupView(
                      ...
                      callBack: (value)async {
                        ...
                        if (value == 0 && cancelBtnTitle == 'ignore'.tr()) {
                          await onTapIgnoreAndSubmit();
                        }
                        if (value == 1 && okBtnTitle == 'refresh'.tr()) {
                          await onTapRefreshAndRetry();
                        }
                      }),
                ),
                Visibility(
                  ...
                  child: LocationAlertPopupView(
                      ...
                      callBack: (value) async{
                        ...
                        if (value == 0 &&
                            locationCancelBtnTitle == 'Cancel'.tr()) {
                          Navigator.pop(context);
                        }
                        if (value == 0 &&
                            locationCancelBtnTitle == 'ignore'.tr()) {
                          await onTapIgnoreAndSubmit();
                        }
                        if (value == 1 &&
                            locationOkBtnTitle == 'refresh'.tr()) {
                          await onTapRefreshAndRetry();
                        }
                      }),
                ),
  ...
  Future<void> updateEscortDutyUI() async {
    if (widget.userAttendance != null &&
        widget.attendanceMode == keyAttendanceModeEscortDuty) {
    ...
    if (approvedEscortDuty.isNotEmpty) {
      ...
      if (widget.attendanceMode == keyAttendanceModeSelf &&
          widget.unitDutyPosts.isNotEmpty) {
  ...
  void onLoadUpdateUI() {
    ...
    userRegNo = widget.userProfile.regNo;
    position = widget.userProfile.symbol;
  }

  void onTapMarkEscortDuty() async{
    await  getUnitPostDataByUnit(escortDutyUnitCode);
  }
  ...
  Future<void> updateLocationData() async {
    ...
    try {
      ...
      if (lastKnown != null) {
        final age = DateTime.now().difference(
            lastKnown.timestamp ?? DateTime.fromMillisecondsSinceEpoch(0));
        if (age.inMinutes <= 5) {
          position = lastKnown; // ✅ Accept only if ≤ 10 min old
        } else {
          printInDebug(
              '⚠️ Last known location is too old: ${age.inMinutes} min');
        }
      }
    }
    ...
    if (position != null) {
      printInDebug(
          '✅ Using Lat: ${position.latitude}, Lng: ${position.longitude}');
  ...
  Future<void> getUnitPostData(String qrId) async {
    print('getUnitPostData...qrId.....$qrId');
    final unitDutyPost =
    widget.unitDutyPosts.where((post) => post.qrId == qrId).toList();
    ...
    if (unitDutyPost.isEmpty) {
      ...
      print('getUnitPostData...internetStatus.....$internetStatus`');

      if (internetStatus) {
        onLoadEmployeeRoasterData(widget.userProfile.regNo);
      } else {
        showPopupAlert('alert'.tr(), 'no_unit_duty_post'.tr());
      }
    } else {
      selectedUnitDutyPost = unitDutyPost.first;
      await getUserPosting(unitDutyPost.first.unitCode);
      // onLoadEmployeeRoasterData(widget.userProfile.regNo);
    }
  }
  ...
  Future<void> getUnitPostDataByUnit(String unitCode) async {
    ...
    final unitDutyPost = widget.unitDutyPosts
        .where((post) => post.unitCode == unitCode)
        .toList();
  ...
  Future<void> getUserPosting(String unitCode) async {
    print('getUserPosting..length...${widget.userPostings.length}');
    ...
    if (userPostings.isEmpty) {
      ...
      if (widget.attendanceStatus == keyAttendanceStatusDutyIn) {
        await getUnitShiftDetailsDutyIn(userPostings.first.unitCode);
      }

      if (widget.attendanceStatus == keyAttendanceStatusDutyOut &&
          widget.userAttendance != null) {
        await getUnitShiftDetailsByIdDutyOut(widget.userAttendance!.shiftId);
      }
    }
  }

  Future<void> getUnitShiftDetailsDutyIn(String unitCode) async {
    print('getUserPosting..getUnitShiftDetailsDutyIn');
    ...
    int doubleDutyCount = await DatabaseHelper.instance
        .countMonthExtraDuties(regNo, dutyDateTime.month, dutyDateTime.year);
    ...
    matchingShiftDetails = widget.unitShiftDetails.where((shift) {
      ...
      Duration shiftStartBeforeDuration =
      _parseShiftDuration(shift.shiftStartBefore);
      Duration shiftEndAfterDuration = _parseShiftDuration(shift.dutyInBefore);

      TimeOfDay adjustedStartTime =
      _adjustTimeOfDay(startTime, shiftStartBeforeDuration);
      TimeOfDay adjustedEndTime =
      _adjustTimeOfDay(endTime, shiftEndAfterDuration);
    ...
    if (matchingShiftDetails.isEmpty) {
      ...
      await  loadSelectShiftView();
    }
  }

  Future<void> getUserPostingFromServer(String unitCode) async {
    print('getUserPosting..length...${userPostingsData.length}');
    final userPostings = userPostingsData.where((userPosting) {
      return userPosting.unitCode == unitCode;
    }).toList();
    ...
    if (userPostings.isEmpty) {
      ...
      selectedUserPosting = userPostings.first;
      if (widget.attendanceStatus == keyAttendanceStatusDutyIn) {
        await getUnitShiftDetailsDutyInFromServer(userPostings.first.unitCode);
      }

      if (widget.attendanceStatus == keyAttendanceStatusDutyOut &&
          widget.userAttendance != null) {
        await getUnitShiftDetailsByIdDutyOutFromServer(
            widget.userAttendance!.shiftId);
      }
    }
  }

  Future<void> getUnitShiftDetailsDutyInFromServer(String unitCode) async {
    DateTime now = DateTime.now();
    TimeOfDay currentTime = TimeOfDay(hour: now.hour, minute: now.minute);
    ...
    // Filter shifts based on time, unitCode, and duty hours condition
    matchingShiftDetails = unitShiftDetails.where((shift) {
      TimeOfDay startTime = _parseTimeOfDay(shift.startTime);
      TimeOfDay endTime = _parseTimeOfDay(shift.endTime);
  ...
  Future<void> getUnitShiftDetailsByIdDutyOut(String shiftId) async {
    ...
    TimeOfDay currentTime = TimeOfDay(hour: now.hour, minute: now.minute);
    print(
        '..getUnitShiftDetailsByIdDutyOut.....${widget.unitShiftDetails.length}');
    final matches = widget.unitShiftDetails.where((shift) {
      ...
      final shiftStartBeforeDuration =
      _parseShiftDuration(shift.shiftStartBefore);
    ...
    await loadSubmitDutyView(dutyInShiftDetail);
  }

  Future<void> getUnitShiftDetailsByIdDutyOutFromServer(String shiftId) async {
    DateTime now = DateTime.now();
    TimeOfDay currentTime = TimeOfDay(hour: now.hour, minute: now.minute);
    print(
        '..getUnitShiftDetailsByIdDutyOut..unitShiftDetails...${unitShiftDetails.length}');
    final matches = unitShiftDetails.where((shift) {
      print('..getUnitShiftDetailsByIdDutyOut..startTime...${shift.startTime}');
      print('..getUnitShiftDetailsByIdDutyOut..endTime...${shift.endTime}');

      final startTime = _parseTimeOfDay(shift.startTime);
      final endTime = _parseTimeOfDay(shift.endTime);
      print(
          '..getUnitShiftDetailsByIdDutyOut..startTime...startTime.....${startTime}');
      print(
          '..getUnitShiftDetailsByIdDutyOut..endTime....endTime....${endTime}');
    ...
    print('..getUnitShiftDetailsByIdDutyOut..matches...${matches.length}');
  ...
  Future<List<EscortDuty>> getApprovedEscortDuty(DateTime selectedDate) async {
    List<EscortDuty> escortDutyAllData =
    await DatabaseHelper.instance.getAllRecords<EscortDuty>(
      keyTableEscortDuty,
  ...
  TimeOfDay _parseTimeOfDay(String timeString) {
    print('..getUnitShiftDetailsByIdDutyOut._parseTimeOfDay....${timeString}');
  ...
  TimeOfDay _adjustTimeOfDay(TimeOfDay time, Duration adjustmentDuration) {
    final adjustmentMinutes =
        adjustmentDuration.inMinutes; // Convert Duration to int (minutes)
  ...
  bool _isTimeBetween(TimeOfDay current, TimeOfDay start, TimeOfDay end) {
    ...
    if (startMinutes <= endMinutes) {
      print('..getUnitShiftDetailsByIdDutyOut.._isTimeBetween...IF...');

      return currentMinutes >= startMinutes && currentMinutes <= endMinutes;
    } else {
      print('..getUnitShiftDetailsByIdDutyOut.._isTimeBetween...ELSE...');
      return currentMinutes >= startMinutes || currentMinutes <= endMinutes;
    }
  }
  ...
  Future<bool> checkDutyLocation(
      UnitDutyPost dutyPost, String currentLatLng, String retryCall) async {
    bool locationVerified = false;

    List<String> activeDayList = currentLatLng.isNotEmpty
    ...
    if (dutyPost.isGeoFenceAllow == 1) {
      ...
      if (currentLat > 0.0 && currentLng > 0.0) {
        int currentDistance =
        getDistanceFromDuty(dutyPost.geoLocation, currentLatLng);
        if (currentDistance <= dutyPost.allowDistance) {
          locationVerified = true;
        } else {
          locationVerified = false;
          String message =
          await getLocationAlertMessage(currentLatLng, currentDistance);
        ...
        showPopupAlert(
          ...
          message,
          cancelText:
          dutyPost.ignoreBlankLocation == 1 ? 'ignore'.tr() : 'Cancel'.tr(),
          okText: 'refresh'.tr(),
        );
      }
    } else {
  ...
  Future<String> getLocationAlertMessage(
      String currentLatLng, int currentDistance) async {
  ...
  Future<void> onTapIgnoreAndSubmit() async {
    if (retryMethod == 'SelectShiftView') {
      await loadSelectShiftView(ignoreAndSubmit: true);
    }

    if (retryMethod == 'SubmitDutyView') {
      await loadSubmitDutyView(dutyInShiftDetail, ignoreAndSubmit: true);
    }
  }
  ...
  void showPopupAlert(String header, String message,
  ...
  void showLocationPopupAlert(String header, String message, String latLng,
      {String cancelText = '', String okText = ''}) {
    setState(() {
  ...
  Future<void> loadSelectShiftView({bool ignoreAndSubmit = false}) async {
    if (!ignoreAndSubmit) {
      if (!await checkDutyLocation(
          selectedUnitDutyPost, deviceLatLong, 'SelectShiftView')) {
        return;
      }
    }
  ...
  void loadShiftOrSubmitDuty(bool ignoreAndSubmit)async {
    locationScannerController.dispose();
    ...
    if (widget.selectedShift == null) {
      ...
      await loadSubmitDutyView(widget.selectedShift,
          ignoreAndSubmit: ignoreAndSubmit);
    }
  }

  Future<void> loadSubmitDutyView(UnitShiftDetail? selectedShift,
      {bool ignoreAndSubmit = false}) async {
    if (!ignoreAndSubmit) {
      if (!await checkDutyLocation(
          selectedUnitDutyPost, deviceLatLong, 'SubmitDutyView')) {
        return;
      }
    }
    Navigator.push(
  ...
  void onLoadEmployeeRoasterData(String userId) {
    ...
    printInDebug(
        'Response:onLoadEmployeeRoasterData....inputData.... ${inputData}');

    APIHelper.instance.getUserData(employeeRoasterApi, inputData, (data) async{
      setState(() {
        showLoaderView = false;
      });
      ...
      if (data.isNotEmpty) {
        ...
        printInDebug('Response:EmployeeRoasterDetail........ ${data}');

        printInDebug(employeeRoaster.employeeDetail.first.empName);
        selectedUnitDutyPost = employeeRoaster.unitDutyPost.first;

        unitDutyPosts = employeeRoaster.unitDutyPost;
        unitShiftDetails = employeeRoaster.unitShiftDetail;
        userPostingsData = employeeRoaster.userPosting;
        print(
            '..getUnitShiftDetailsByIdDutyOut.userPostingsData....${userPostingsData.length}');

        print(
            '..getUnitShiftDetailsByIdDutyOut.unitShiftDetails.a....a.......${unitShiftDetails.length}');

        await getUserPostingFromServer(employeeRoaster.unitDutyPost.first.unitCode);
      } else {
        if (widget.attendanceStatus == keyAttendanceStatusDutyOut) {
          showPopupAlert('alert'.tr(), 'other_mark_shift_not_allowed1'.tr());
        } else {
